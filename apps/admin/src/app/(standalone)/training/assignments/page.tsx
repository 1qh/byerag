...
const renderAssignmentStatus = (r: { overdueDays: number; status: string }): React.ReactElement => {
  if (r.status === 'passed') return <span className='text-green-600'>✓ Passed</span>
  if (r.status === 'overdue')
    return (
  ...
  return <span className='text-muted-foreground'>Not passed</span>
}
  ...
  const renderRows = (): React.ReactElement[] => {
    ...
    return at.rows.map(r => (
      <TableRow key={`${r.userId}-${r.test}-${r.at}`}>
        ...
        <TableCell>{renderAssignmentStatus(r)}</TableCell>
        <TableCell
          className={cn(
