            ...
            <tr key={r._id} className="border-b">
              ...
              <td>
                <select
                  ...
                  value={r.role}
                  onChange={e => { setRole({ role: e.target.value as 'admin' | 'user', userId: r.userId }).catch((err: unknown) => { alert(String(err)) }) }}>
              ...
              <td>
                <select
                  className="rounded border px-2 py-1"
                  value={r.department ?? ''}
                  onChange={e => { setDept({ department: (e.target.value || undefined) as 'HR' | 'Sales' | 'IT' | undefined, userId: r.userId }).catch((err: unknown) => { alert(String(err)) }) }}>
                  <option value="">—</option>
                  {DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
              </td>
              <td className="text-muted-foreground text-xs">{new Date(r.updatedAt).toISOString().slice(0, 10)} by {r.updatedBy}</td>
            </tr>
          ))}
        </tbody>
      </table>
