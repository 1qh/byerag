...
const action = defineTool({
  ...
  handler: async (ctx, args) => {
    ...
    if (!rowB) throw fail({ code: 'NOT_FOUND', message: `doc ${args.b} not found or no extracted text` })
    ...
    try {
      ...
      throw fail({ code: 'UPSTREAM_ERROR', message: `kimi: ${String(error).slice(0, 120)}` })
    }
