...
const { auth, isAuthenticated, signIn, signOut, store } = convexAuth({
  ...
  jwt: {
    customClaims: async (ctx, { userId }) => {
      const user = (await ctx.db.get(userId)) as null | { email?: string }
      ...
      ? [
          ...
          Anonymous<DataModel>({
            profile: params => {
              ...
              if (!EMAIL_RE.test(email)) throw new Error('valid email param required for dev sign-in')
              return { email, isAnonymous: true }
            }
          })
        ]
      : [Google]
