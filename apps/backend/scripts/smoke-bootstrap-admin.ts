...
const a = (await c.mutation(api.testing.createOrUpdateUserProbe, {
  bootstrapAdmins: admins,
  email: ADMIN,
  testSecret
})) as { role: string }
check('first sign-in for BOOTSTRAP_ADMIN → role=admin', a.role === 'admin', JSON.stringify(a))
const p = (await c.mutation(api.testing.createOrUpdateUserProbe, {
  bootstrapAdmins: admins,
  email: PLAIN,
  testSecret
})) as { role: string }
check('first sign-in for non-bootstrap → role=user', p.role === 'user', JSON.stringify(p))
const a2 = (await c.mutation(api.testing.createOrUpdateUserProbe, {
  bootstrapAdmins: admins,
  email: ADMIN,
  testSecret
})) as { role: string }
