#!/usr/bin/env bun
/* eslint-disable no-console */
/** biome-ignore-all lint/performance/noAwaitInLoops: sequential by design */
/** biome-ignore-all lint/style/noProcessEnv: smoke reads .env directly */
/** biome-ignore-all lint/nursery/noUndeclaredEnvVars: smoke env */
import { ConvexHttpClient } from 'convex/browser'
import { readFileSync } from 'node:fs'
import { join } from 'node:path'
import { api } from '../convex/_generated/api'
const ENV_LINE = /^\s*(?<key>[A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?<val>.*?)\s*$/u
const parseEnv = (text: string): Record<string, string> => {
  const out: Record<string, string> = {}
  for (const line of text.split('\n')) {
    const t = line.trim()
    if (t && !t.startsWith('#')) {
      const m = ENV_LINE.exec(line)
      const key = m?.groups?.key
      if (key) out[key] = (m.groups?.val ?? '').replaceAll(/^["']|["']$/gu, '')
    }
  }
  return out
}
const env = parseEnv(readFileSync(join(import.meta.dir, '..', '.env'), 'utf8'))
const url = env.CONVEX_SELF_HOSTED_URL ?? ''
const testSecret = env.TEST_SECRET ?? ''
if (!(url && testSecret)) {
  console.error('env missing')
  process.exit(2)
}
const c = new ConvexHttpClient(url)
let pass = 0
let fail = 0
const check = (label: string, ok: boolean, detail: string): void => {
  console.log(`${ok ? '✓' : '✗'} ${label}: ${detail}`)
  if (ok) pass += 1
  else fail += 1
}
const ADMIN = 'regen-admin@example.com'
console.log('[regen] wipe + seed topic pool=5')
await c.mutation(api.testing.wipeTrainingTables, { testSecret })
const topicId = await c.mutation(api.testing.seedTopicWithPool, { name: 'Regen', poolSize: 5, testSecret })
const qids = (await c.query(api.testing.listQuestionsForTopic, { testSecret, topicId: topicId as never })) as {
  _id: string
}[]
const qid = qids[0]?._id
if (!qid) {
  console.error('no question')
  process.exit(1)
}
console.log('[regen] regenerate 5 times (cap)')
for (let i = 1; i <= 5; i += 1) {
  const r = (await c.mutation(api.testing.regenerateQuestionProbe, {
    adminEmail: ADMIN,
    hint: `pass ${i}`,
    questionId: qid as never,
    testSecret
  })) as { regenCount: number }
  check(`regen ${i} returns regenCount=${i}`, r.regenCount === i, `regenCount=${r.regenCount}`)
}
console.log('[regen] 6th regen → must reject')
let capped = false
let msg = ''
try {
  await c.mutation(api.testing.regenerateQuestionProbe, {
    adminEmail: ADMIN,
    hint: 'past cap',
    questionId: qid as never,
    testSecret
  })
} catch (error) {
  capped = true
  msg = String(error)
}
check('regen 6 rejected at cap', capped && msg.includes('cap'), `capped=${capped} msg=${msg.slice(0, 80)}`)
console.log(`\n[regen] SUMMARY pass=${pass} fail=${fail} total=6`)
if (fail > 0) process.exit(1)
