...
const waitExtracted = async (
  id: string,
  expectedToken: string,
  deadlineMs: number
): Promise<{ extractedTextLen: number; foundToken: boolean }> => {
...
if (r1.extractedTextLen === 0) {
  console.error('FAIL text/plain extract empty')
  process.exit(1)
}
...
if (!r2.foundToken) {
  console.error('FAIL docx extract did not surface fixture token')
  process.exit(1)
}
